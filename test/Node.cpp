...
BOOST_AUTO_TEST_CASE(AddNode) {
	...
	/* Add node */
	BOOST_REQUIRE_NO_THROW(fx.lux.addnode("127.0.0.255", "add"));

	std::vector<nodeinfo_t> nodeVec;
	BOOST_REQUIRE_NO_THROW(nodeVec = fx.lux.getaddednodeinfo(false));
...
BOOST_AUTO_TEST_CASE(GetAddedNodeInfo) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getaddednodeinfo(true, "127.0.0.255"));
...
BOOST_AUTO_TEST_CASE(RemoveNode){
	...
	BOOST_REQUIRE_NO_THROW(fx.lux.addnode("127.0.0.255", "remove"));
...
BOOST_AUTO_TEST_CASE(ConnectionCount){
	...
	int count;
	BOOST_REQUIRE_NO_THROW(count = fx.lux.getconnectioncount());
...
BOOST_AUTO_TEST_CASE(GetPeerInfo){
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getpeerinfo());
