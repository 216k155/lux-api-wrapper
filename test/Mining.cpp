...
BOOST_AUTO_TEST_CASE(GetBestBlockHash) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getbestblockhash());
...
BOOST_AUTO_TEST_CASE(GetBlockHash) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getblockhash(300000));
	BOOST_REQUIRE(response.size() == 64);
...
BOOST_AUTO_TEST_CASE(GetBlockCount) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getblockcount());
...
BOOST_AUTO_TEST_CASE(GetBlock) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getblock("000000000001fdd87c8e502a26068769f4d54d6e5122b8367019e47a1eef728a"));
...
BOOST_AUTO_TEST_CASE(GetGenerate) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getgenerate());
...
BOOST_AUTO_TEST_CASE(SetGenerate) {
	...
	BOOST_REQUIRE_NO_THROW(fx.lux.setgenerate(true));
	BOOST_REQUIRE(fx.lux.getgenerate() == true);

	BOOST_REQUIRE_NO_THROW(fx.lux.setgenerate(false));
	BOOST_REQUIRE(fx.lux.getgenerate() == false);
}
...
BOOST_AUTO_TEST_CASE(GetDifficulty) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getdifficulty());
...
BOOST_AUTO_TEST_CASE(GetHashesPerSec) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.gethashespersec());
...
BOOST_AUTO_TEST_CASE(GetMiningInfo) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getmininginfo());
...
BOOST_AUTO_TEST_CASE(GetWork) {
	...
	std::string param =
			"000000000001fdd87c8e502a26068769f4d54d6e5122b8367019e4"
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.getwork());
	...
	BOOST_REQUIRE_NO_THROW(response2 = fx.lux.getwork(param));
...
BOOST_AUTO_TEST_CASE(ListSinceBlock) {
	...
	BOOST_REQUIRE_NO_THROW(response = fx.lux.listsinceblock("", 1));
